...
async function getDades (req, res) {
  ...
  if (receivedPOST) {
    ...
    if (receivedPOST.type == "cpus"){
      var objCPUList = objConsolesList.map((obj) => { return obj.processor })
      await wait(1500)
      let senseDuplicats = [...new Set(objCPUList)]
      result = { status: "OK", result: senseDuplicats.sort() } 
    }
    if (receivedPOST.type == "consoles"){
      var objConsolesList = objConsolesList.map((obj) => { return obj })
      await wait(1500)
      let senseDuplicats = [...new Set(objConsolesList)]
      result = { status: "OK", result: senseDuplicats.sort() } 
    }
    if (receivedPOST.type == "color") {
